...
export const callback = async (authCode, dispatch, push) => {
  const resetURI =
  `${import.meta.env.VITE_MAP_ROULETTE_SERVER_URL}/auth/callback?code=${authCode}`
...
export const resetAPIKey = function(userId) {
  return function(dispatch) {
    const resetURI =
      `${import.meta.env.VITE_MAP_ROULETTE_SERVER_URL}/auth/generateAPIKey?userId=${userId}`
...
export const logoutUser = function(userId) {
  ...
  const logoutURI = `${import.meta.env.VITE_MAP_ROULETTE_SERVER_URL}/auth/signout`
