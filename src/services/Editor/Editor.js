...
export const editTask = function (
  ...
  return function (dispatch) {
    if (options && import.meta.env.VITE_FEATURE_EDITOR_IMAGERY !== "enabled") {
...
export const constructIdURI = function (task, mapBounds, options, taskBundle, replacedComment) {
  const baseUriComponent = `${import.meta.env.VITE_ID_EDITOR_SERVER_URL}?editor=id`;
...
export const constructRapidURI = function (task, mapBounds, options, replacedComment) {
  const baseUriComponent = `${import.meta.env.VITE_RAPID_EDITOR_SERVER_URL}#`;
...
export const constructLevel0URI = function (
  ...
  const baseUriComponent = `${import.meta.env.VITE_LEVEL0_EDITOR_SERVER_URL}?`;
...
export const josmImportCooperative = function (
  ...
  return josmImportURI(
    ...
    mapBounds,
    taskBundle,
    `${import.meta.env.VITE_MAP_ROULETTE_SERVER_URL}/api/v2/task/${task.id}/cooperative/change/task_${task.id}_change.osc`,
...
export const josmImportReferenceLayers = function (
  ...
  return _map(referenceLayers, (layer) => {
    ...
    return josmImportURI(
      ...
      mapBounds,
      taskBundle,
      `${import.meta.env.VITE_MAP_ROULETTE_SERVER_URL}/api/v2/task/${task.id}/attachment/${layer.id}/data/${filename}`,
      {
