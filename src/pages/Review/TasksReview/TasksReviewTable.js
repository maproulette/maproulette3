...
export class TaskReviewTable extends Component {
  ...
  updateTaskStatusFiltersByCategory = (filterItem, category) => {
    if(!this.state[category]) return
    let newIds = this.state[category].slice()
    if(this.state[category].includes(filterItem.value)) {
      newIds = newIds.filter(i => i !== filterItem.value)
    } else {
  ...
  render() {
    ...
    const columnTypes = setupColumnTypes({
                            ...
                            taskPriorityFilterIds: this.state.taskPriorityFilterIds
                           },
