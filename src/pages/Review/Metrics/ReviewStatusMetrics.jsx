...
export default class ReviewStatusMetrics extends Component {
  buildReviewStats = (type, metrics) => {
    ...
    return (
      <div className="mr-grid mr-grid-columns-1 mr-grid-gap-4">
        {type === ReviewTasksType.toBeReviewed && this.buildToBeReviewed(metrics)}

        {type === ReviewTasksType.reviewedByMe && !asMetaReview && this.buildReviewedByMe(metrics)}
        ...
        {type === ReviewTasksType.metaReviewTasks &&
          !asMetaReview &&
  ...
  buildToBeReviewed = (metrics) => {
    return (
      <Fragment>
        {buildMetric(
          metrics.reviewRequested,
          metrics.total,
          <FormattedMessage {...messages.awaitingReview} />,
        )}
        {buildMetric(
          metrics.reviewDisputed,
          metrics.total,
  ...
  buildMetaReviewTasksAsMetaReviewStatus = (metrics) => {
    return (
      <Fragment>
        {buildMetric(
          metrics.total - metrics.metaReviewRequested,
          metrics.total,
      ...
      if (reviewMetricsByPriority?.[priority]) {
        ...
        return (
          <div className="mr-mt-6 mr-mb-6" key={priority}>
            <div className={classNames("mr-text-md mr-mb-4", "mr-text-yellow mr-font-normal")}>
              <FormattedMessage
