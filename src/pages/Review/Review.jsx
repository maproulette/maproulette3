...
import { generateWidgetId, WidgetDataTarget, widgetDescriptor } from "../../services/Widget/Widget";
...
export const defaultWorkspaceSetup = function () {
  return {
    ...
    layout: [{ i: generateWidgetId(), x: 0, y: 0, w: 12, h: 18 }],
    excludeWidgets: [],
  };
};
...
export class ReviewTasksDashboard extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (user.isReviewer()) {
      if (
        ...
        _get(this.props, "match.params.showType") !== this.state.showType &&
        !_isUndefined(_get(this.props, "match.params.showType"))
      ) {
    ...
    if (!_isEqual(this.props.location.search, prevProps.location.search)) {
      this.setSelectedFilters(buildSearchCriteriafromURL(this.props.history.location.search), true);
      return;
    }
  ...
  render() {
    ...
    const metaReviewEnabled = window.env.REACT_APP_FEATURE_META_QC === "enabled";
      ...
      <div>
        ...
        <ol className="mr-list-reset mr-text-md mr-leading-tight mr-flex">
          ...
          <li className="mr-ml-4 mr-border-l mr-pl-4 mr-border-green">
            <button
              className={classNames(
                showType === ReviewTasksType.reviewedByMe
                  ? "mr-text-white"
          ...
          <li className="mr-ml-4 mr-border-l mr-pl-4 mr-border-green">
            <button
              className={classNames(
                showType === ReviewTasksType.myReviewedTasks
                  ? "mr-text-white"
                  : "mr-text-green-lighter",
              )}
          ...
          <li className="mr-ml-4 mr-border-l mr-pl-4 mr-border-green">
            <button
              className={classNames(
                showType === ReviewTasksType.allReviewedTasks
                  ? "mr-text-current"
                  : "mr-text-green-lighter",
              )}
          ...
          {metaReviewEnabled && (
            <li className="mr-ml-4 mr-border-l mr-pl-4 mr-border-green">
              <button
                className={classNames(
                  showType === ReviewTasksType.metaReviewTasks
                    ? "mr-text-current"
                    : "mr-text-green-lighter",
                )}
        ...
        {showType === ReviewTasksType.reviewedByMe && metaReviewEnabled && (
          <ol className="mr-list-reset mr-text-md mr-leading-tight mr-flex mr-mt-6">
            <li className="mr-text-yellow">{this.props.intl.formatMessage(messages.role)}</li>
            ...
            <li className="mr-ml-4 mr-pl-4 mr-border-green">
              <button
                className={classNames(
                  this.state.showSubType === "meta-reviewer"
                    ? "mr-text-white"
                    : "mr-text-green-lighter",
                )}
    ...
    const notReviewerTabs = (
      <div>
        <ol className="mr-list-reset mr-text-md mr-leading-tight mr-flex mr-links-green-lighter">
          <li>{this.props.intl.formatMessage(messages.myReviewTasks)}</li>
          <li className="mr-ml-4 mr-border-l mr-pl-4 mr-border-green">
