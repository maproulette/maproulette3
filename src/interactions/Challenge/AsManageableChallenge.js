...
export class AsManageableChallenge {
  ...
  isComplete() {
    return (
      ...
      (_get(this, "actions.total", 0) > 0 && _get(this, "actions.available") === 0)
    );
  }

  completionPercentage() {
    if (_get(this, "actions.total", 0) > 0) {
      return percentage(this.actions.total, this.actions.total - this.actions.available);
    } else {
      return 0;
    }
  }
  ...
  actionPercentage(action) {
    return percentage(_get(this, "actions.total"), _get(this, `actions.${action}`, 0));
  }

  completionActivity() {
    return _filter(this.activity, (entry) => entry.count > 0 && isCompletionStatus(entry.status));
  }
  ...
  mostRecentCompletionActivityEntry() {
    ...
    return _maxBy(this.completionActivity(), (entry) => parseISO(entry.date).getTime());
  }
