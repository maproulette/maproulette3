...
import { statusLabels, keysByStatus } from "../../../services/Task/TaskStatus/TaskStatus";
...
export default class UserActivityTimelineWidget extends Component {
  ...
  render() {
    ...
    const decoratedActivities = _compact(
      _map(this.props.user.activity, (entry) => {
        ...
        return Object.assign({}, entry, {
          ...
          description:
            `${localizedActionLabels[keysByAction[entry.action]]} ` +
        ...
        return (
          <li key={challengeId}>
            <h2 className="mr-timeline__name mr-links-green-lighter">
              <Link to={`/browse/challenges/${challengeId}`}>{challengeName}</Link>
            </h2>
            <ol className="mr-timeline__entries">{activityEntries}</ol>
          </li>
        );
      });
        ...
        <li key={entriesByDate[0]} className="mr-timeline__period">
          <h3 className="mr-timeline__header">
            {this.props.intl.formatDate(
              parseISO(format(entriesByDate[0], "yyyy-MM-dd'T'HH:mm:ssxxx")),
              { month: "long", day: "numeric" },
            )}
          </h3>
          <ol className="mr-timeline__activity">{challengeActivity}</ol>
        </li>
      );
    });
    ...
    return (
      ...
      >
        <ol className="mr-timeline dark-mode">{timelineItems}</ol>
      </QuickWidget>
    );
  }
}
