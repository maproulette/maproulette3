...
import { FormattedMessage, FormattedTime, FormattedDate, injectIntl } from "react-intl";
...
const OSMElementHistory = (props) => {
  ...
  const featureIds = _flatten(
    _compact(
      _map(allTasks, (task) => {
        const geometries = AsMappableTask(task).normalizedGeometries();
        return geometries
          ? _compact(
              _map(geometries.features, (f) =>
                AsIdentifiableFeature(f).normalizedTypeAndId(true, "/"),
              ),
            )
          : null;
      }),
  ...
  useEffect(() => {
    ...
    setFetchErr(null);
    fetchOSMElementHistory(activeFeatureId, true)
      .then((historyEntries) => {
  ...
  if (!history) {
    return <FormattedMessage {...messages.noOSMElements} />;
  }
  ...
  const featureChangeset = featureProperties.osmVersion || featureProperties.last_edit_changeset;
...
const HistoryEntry = (props) => {
  ...
  const changesetComment = _find(_get(props, "changeset.tag", []), (tag) => tag.k === "comment");
  ...
  return (
    <li className="mr-mb-4 mr-text-white mr-text-sm">
      <div className="mr-flex mr-justify-between mr-text-xs mr-font-bold mr-mb-1">
        <div>
          <FormattedTime value={timestamp} hour="2-digit" minute="2-digit" />,{" "}
          <FormattedDate value={timestamp} year="numeric" month="long" day="2-digit" />
        </div>
      ...
      <div className="mr-rounded-sm mr-p-2 mr-bg-black-15 mr-relative">
        ...
        <div className="mr-mb-2" style={{ color: AsColoredHashable(props.user).hashColor }}>
          {props.user}
        </div>
...
const FeatureSelectionDropdown = (props) => {
  const menuItems = _map(props.featureIds, (featureId) => (
    <li key={featureId}>
      <a onClick={() => props.selectFeatureId(featureId)}>{featureId}</a>
    </li>
  ));
      ...
      dropdownButton={(dropdown) => (
        <Fragment>
          <a className="mr-flex" onClick={dropdown.toggleDropdownVisible}>
            <div className="mr-mr-2">{props.selectedFeatureId}</div>
            <SvgSymbol
