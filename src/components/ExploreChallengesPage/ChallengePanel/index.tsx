...
export const ChallengePanel = ({ viewMode = 'card', showMap = false }: ChallengePanelProps) => {
  ...
  return (
    <div className={`relative flex h-full w-full flex-1 flex-col overflow-hidden bg-white ${showMap ? 'md:w-120 md:rounded-bl-lg md:border-zinc-200 md:border-r' : 'md:rounded-b-lg'} dark:border-zinc-800 dark:bg-zinc-950`}>
      <div
      ...
      {!challenges || challenges.length === 0 ? (
        ...
        <ScrollArea className="h-full w-full">
          <div className={`w-full gap-4 p-4 ${showMap ? 'flex flex-col' : 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5'}`}>
            {challenges.map((c) => (
              <ChallengeCard key={c.id} challenge={c} className={showMap ? 'min-w-full' : 'min-w-[280px]'} />
            ))}
          </div>
        </ScrollArea>
      )}
