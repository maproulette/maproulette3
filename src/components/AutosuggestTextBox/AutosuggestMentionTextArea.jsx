...
export default class AutosuggestMentionTextArea extends Component {
  ...
  inputChanged = (inputText, downshift) => {
    if (!this.props.disableShowSuggestions) {
      const searchOn = this.findMatch(inputText, this.props.inputRef?.current?.selectionStart);
    ...
    if (this.props.onInputValueChange && !downshift.selectedItem && this.state.textBoxActive) {
  ...
  replaceMatch = (input, selectedValue, cursorPosition) => {
    ...
    if (searchMatch) {
      ...
      const index =
        searchMatch[1] && searchMatch[1] !== "[" ? searchMatch.index + 1 : searchMatch.index;

      return [input.slice(0, index), "[@", selectedValue, "]", input.slice(cursorPosition)].join(
        "",
      );
    }
  };
  ...
  dropdownItems(getItemProps) {
    ...
    if (!_isEmpty(preferredResults)) {
      ...
      items = items.concat(
        _map(preferredResults, (result, index) => {
    ...
    items = items.concat(
      _map(searchResults, (result, index) =>
        generateResult(result, "", index + preferredResults.length),
      ),
    );
    return items;
  }
  ...
  getSearchResults = () => {
    ...
    // in the list twice.
    return _differenceBy(this.props.searchResults, this.props.preferredResults, "displayName");
  };
  ...
  render() {
    return (
      ...
      >
        {(downshift) => {
          const searchOn = this.findMatch(
            downshift.inputValue,
            this.props.inputRef?.current?.selectionStart,
          );
          const resultItems =
            searchOn !== undefined && searchOn !== null
              ? this.dropdownItems(downshift.getItemProps, searchOn)
              : null;
