...
const limitUserResults = (challenges) => {
  const ownerLimit = Number(import.meta.env.VITE_BROWSE_CHALLENGES_OWNER_LIMIT);
...
export class ChallengeResultList extends Component {
    ...
    async fetchData() {
      if (this.state.pauseFetch) {
        ...
        if (query) {
          try {
            const response = await fetch(`${import.meta.env.VITE_MAP_ROULETTE_SERVER_URL}/api/v2/task/${query}`);
