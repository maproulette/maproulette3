...
const ListFilterItems = function (props) {
  ...
  const menuItems = categories.map((keyword) => (
    <li className="mr-flex" key={keyword}>
      <a
        className={`mr-flex-1 ${categorizationFilters.includes(keyword) ? "mr-text-yellow" : ""}`}
        onClick={() => props.updateFilter(keyword, categorizationFilters)}
      >
        <div className={categorizationFilters.includes(keyword) ? "mr-text-yellow" : ""}>
          {keyword}
        </div>
      </a>
  ...
  menuItems.push(
    <li key="add">
      ...
      <div className="mr-flex mr-items-center mr-py-3">
        <label htmlFor="input-name" className="mr-text-green-lighter mr-mr-4 mr-cursor-pointer">
