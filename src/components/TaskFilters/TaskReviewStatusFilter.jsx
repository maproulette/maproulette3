...
export default class TaskReviewStatusFilter extends Component {
  render() {
    const taskReviewStatusFiltersActive =
      !Object.values(this.props.includeTaskReviewStatuses).every((value) => value) ||
      Object.keys(this.props.includeTaskReviewStatuses).length <
    ...
    const taskMetaReviewStatusFiltersActive = this.props.metaReviewEnabled
      ? Object.keys(this.props.includeMetaReviewStatuses).length <
        Object.keys(TaskMetaReviewStatusWithUnset).length
      : false;

    const areTaskReviewStatusFilersActive = this.props.metaReviewEnabled
      ? taskReviewStatusFiltersActive ||
        !currentTaskMetaReviewStatuses ||
        taskMetaReviewStatusFiltersActive
      : taskReviewStatusFiltersActive;

    const metaReviewStatusFilter = !this.props.metaReviewEnabled
      ? {}
      : {
          secondaryFilterLabel: this.props.intl.formatMessage(messages.filterByReviewStatusLabel),
          secondaryFilters: _map(TaskMetaReviewStatusWithUnset, (status) => (
            <li key={`meta-${status}`}>
    ...
    return (
      <div className="mr-flex mr-space-x-1 mr-items-center">
        ...
        <FilterDropdown
          ...
          filters={_map(TaskReviewStatusWithUnset, (status) => (
