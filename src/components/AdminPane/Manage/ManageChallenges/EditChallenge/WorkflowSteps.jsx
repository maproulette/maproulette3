...
import {
  jsSchema as automatedEditsCodeAgreementJsSchema,
  uiSchema as automatedEditsCodeAgreementUiSchema,
} from "./Schemas/AutomatedEditsCodeAgreementSchema";
import { jsSchema as basemapJsSchema, uiSchema as basemapUiSchema } from "./Schemas/BasemapSchema";
import { jsSchema as step1jsSchema, uiSchema as step1uiSchema } from "./Schemas/DataSourceSchema";
import { jsSchema as step2jsSchema, uiSchema as step2uiSchema } from "./Schemas/DescriptionSchema";
import {
  jsSchema as discoverabilityJsSchema,
  uiSchema as discoverabilityUiSchema,
} from "./Schemas/DiscoverabilitySchema";
import { jsSchema as editorJsSchema, uiSchema as editorUiSchema } from "./Schemas/EditorSchema";
import { jsSchema as step3jsSchema, uiSchema as step3uiSchema } from "./Schemas/InstructionsSchema";
import {
  jsSchema as osmCommitJsSchema,
  uiSchema as osmCommitUiSchema,
} from "./Schemas/OSMCommitSchema";
import {
  jsSchema as prioritiesJsSchema,
  uiSchema as prioritiesUiSchema,
} from "./Schemas/PrioritiesSchema";
import {
  jsSchema as propertiesJsSchema,
  uiSchema as propertiesUiSchema,
} from "./Schemas/PropertiesSchema";
import { jsSchema as tagsJsSchema, uiSchema as tagsUiSchema } from "./Schemas/TagsSchema";
import { jsSchema as zoomJsSchema, uiSchema as zoomUiSchema } from "./Schemas/ZoomSchema";
...
const allOptionsStep = {
  id: "AllOptions",
  component: (props) => <MenuStep {...props} headerMessage={messages.allOptionsStepDescription} />,
  icon: "options-icon",
  viewBox: "0 0 20 20",
};
...
const newChallengeSteps = {
  ...
  AdvancedOptions: Object.assign({}, advancedOptionsStep, {
    next: ["Discoverability", "Priorities", "Zoom", "Basemap", "Properties", "Tags", "Editor"],
...
export const validateStepTransition = (activeStep, toStepId, allowedStep, transitionName) => {
  const transitionError = new Error(
    `Invalid ${transitionName} step "${toStepId}" for step ${activeStep.id}`,
  );
...
const WorkflowSteps = (props) => {
  ...
  const prevStep = (stepId) => {
    ...
    transitionToStep(_isString(activeStep.previous) ? activeStep.previous : stepId);
  };
  ...
  const nextStep = (stepId) => {
    if (!_isEmpty(activeStep.next)) {
      ...
      transitionToStep(_isString(activeStep.next) ? activeStep.next : stepId);
    } else {
      props.finish();
    }
  };
