...
export const jsSchema = (
  ...
  const remoteUrl = {
    properties: {
      ...
      remoteGeoJson: {
        title: " ",
        description: sourceReadOnly
          ? intl.formatMessage(messages.remoteGeoJsonReadOnly)
          : undefined,
        type: "string",
      },
      dataOriginDate: {
  ...
  if (!sourceReadOnly) {
    schema.properties.source = {
      ...
      type: "string",
      enum: ["Overpass Query", "Local File", "Remote URL"],
      enumNames: [
    ...
    schema.dependencies = {
      source: {
        oneOf: [overpass, localUpload, remoteUrl],
      },
    };
  ...
  return {
    email: {
      ...
      "ui:groupHeader": options.longForm
        ? intl.formatMessage(messages.contactInfoStepHeader)
        : undefined,
    },
    name: {
      "ui:help": intl.formatMessage(messages.nameDescription),
      "ui:groupHeader": options.longForm
        ? intl.formatMessage(messages.dataSourceStepHeader)
        : undefined,
    },
    source: {
