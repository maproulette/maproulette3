...
describe("preparePriorityRuleGroupForSaving", () => {
  ...
  test("no rules generates empty stringified object", () => {
    ...
    expect(preparePriorityRuleGroupForSaving(basicFormGroup)).toEqual(JSON.stringify({}));
  });
  ...
  test("condition and rules are added into results, with dot-separated key and value", () => {
    const result = JSON.parse(preparePriorityRuleGroupForSaving(basicFormGroup));
  ...
  test("empty values are represented as a space in dot-separated string", () => {
    ...
    expect(result.rules[0].value).toEqual(`${basicFormGroup.rules[0].key}. `);
...
describe("preparePriorityRuleGroupForForm", () => {
  ...
  beforeEach(() => {
    ...
    basicSavedGroup = {
      condition: "AND",
      rules: [equalsOperatorSavedRule, containsOperatorSavedRule],
    };
  });
  ...
  test("missing rules generates empty rules", () => {
    ...
    expect(preparePriorityRuleGroupForForm(basicSavedGroup).ruleGroup.rules.length).toBe(0);
  });
