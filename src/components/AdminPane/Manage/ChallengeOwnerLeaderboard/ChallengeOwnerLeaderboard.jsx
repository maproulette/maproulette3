...
export default class ChallengeOwnerLeaderboard extends Component {
  render() {
    ...
    const showNumberTasks =
      this.props.leaderboard.length > 0 ? this.props.leaderboard[0].completedTasks > 0 : false;

    const leaders = _map(this.props.leaderboard.slice(0, 9), (leader) => {
      const avgTimeSpent =
        leader.avgTimeSpent && leader.avgTimeSpent > 0
    ...
    return (
      <Fragment>
        ...
        <ol className="mr-list-reset mr-links-green-lighter mr-text-lg">{leaders}</ol>
      </Fragment>
    );
  }
}
