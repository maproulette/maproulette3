...
export default class ProjectManagersWidget extends Component {
  ...
  updateManagerRole = (manager, role) => {
    ...
    const updateManager =
      ...
      this.props.setProjectManagerRole(
        this.props.project.id, manager.osmId, true, role
      )

    updateManager.then(() => this.setState({
  ...
  removeManager = (manager) => {
    ...
    removeManager.then(() => this.setState({
      updatingManagers: _without(this.state.updatingManagers, managerId)
    }))
  }
  ...
  addManager = role => {
    ...
    const addManager = this.state.choosingTeam ?
      this.props.setTeamProjectRole(
        this.props.project.id,
        this.state.addManagerObject.id,
        role
      ) :
      this.props.addProjectManager(
        this.props.project.id, this.state.addManagerName, role
      )

    addManager.then(() => this.setState({
      addManagerName: '',
      addManagerObject: null,
      addingManager: false,
    }))
  ...
  render() {
    ...
    const adminManagers =
      _filter(this.props.project.managers.concat(this.props.project.teamManagers),
