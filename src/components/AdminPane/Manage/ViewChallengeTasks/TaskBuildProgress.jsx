...
const TaskStatusOverview = ({ actions }) => {
  return (
    <div>
      <div>
        <FormattedMessage {...messages.actionCreated} />: {actions.available}
      </div>
      <div>
        <FormattedMessage {...messages.actionFixed} />: {actions.fixed}
      </div>
      <div>
        <FormattedMessage {...messages.actionNotAnIssue} />: {actions.falsePositive}
      </div>
      <div>
        <FormattedMessage {...messages.actionSkipped} />: {actions.skipped}
      </div>
      <div>
        <FormattedMessage {...messages.actionAlreadyFixed} />: {actions.alreadyFixed}
      </div>
      <div>
        <FormattedMessage {...messages.actionTooHard} />: {actions.tooHard}
      </div>
      <div>
        <FormattedMessage {...messages.actionDisabled} />: {actions.disabled}
      </div>
    </div>
  );
};
...
export default class TaskBuildProgress extends Component {
  ...
  nextUpdateSeconds = () => {
    const secondsRemaining =
      (new Date(this.state.lastTimerRun + TIMER_INTERVAL).getTime() - Date.now()) / 1000;
          ...
          <div>
            <div className="mr-flex mr-justify-between mr-items-center mr-w-full mr-mb-8">
              <h3 className="mr-text-white">
                <FormattedMessage {...messages.tasksUpdating} /> <BusySpinner inline />
              </h3>

              <div>
                <FormattedMessage {...messages.totalElapsedTime} />{" "}
            ...
            <div className="mr-text-lg">
              ...
              <span className="mr-text-orange mr-ml-1">
                <FormattedDuration seconds={this.nextUpdateSeconds()} format={TIMER_FORMAT} />
              </span>
            </div>
    ...
    return (
      <div>
        <div>
          <div className="mr-flex mr-justify-between mr-items-center mr-w-full mr-mb-8">
            <h3 className="mr-text-white">
              <FormattedMessage {...messages.tasksBuilding} /> <BusySpinner inline />
            </h3>
          ...
          <div className="mr-text-lg">
            <span className="mr-text-xl mr-text-pink">{taskCount}</span>{" "}
