...
export const WithProjectReviewMetrics = function (WrappedComponent) {
  return class extends Component {
    ...
    refreshMetrics(props) {
      ...
      props.refreshReviewMetrics(props.user?.id, challengeIds, props.project?.id).then(() => {
    ...
    componentDidUpdate() {
      if (
        ...
        !_isEqual(this.getChallengeIds(this.props), this.state.currentChallengeIds)
      ) {
...
const mapDispatchToProps = (dispatch) => ({
  refreshReviewMetrics: (userId, challengeIds, projectId) => {
    return dispatch(
      fetchReviewMetrics(userId, ReviewTasksType.allReviewedTasks, {
...
export default (WrappedComponent) =>
  connect(
    mapStateToProps,
    mapDispatchToProps,
  )(WithCurrentUser(WithProjectReviewMetrics(WrappedComponent)));
