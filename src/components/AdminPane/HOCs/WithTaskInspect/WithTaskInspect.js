 ...
 */
const WithTaskInspect = (WrappedComponent) => connect(null, mapDispatchToProps)(WrappedComponent);
...
export const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    ...
    previousSequentialTask: (task) => {
      dispatch(loadPreviousSequentialTaskFromChallenge(task.parent.id, task.id)).then((newTask) =>
    ...
    nextSequentialTask: (task) => {
      dispatch(loadNextSequentialTaskFromChallenge(task.parent.id, task.id)).then((newTask) =>
        inspectNewTask(task, newTask, ownProps.history),
      );
    },
...
export const inspectNewTask = function (currentTask, newTask, history) {
  ...
  if (_isObject(newTask) && newTask.id !== currentTask.id) {
    history.push(
      `/admin/project/${projectId}` + `/challenge/${challengeId}/task/${newTask.id}/inspect`,
    );
  } else {
