...
import { Control, DomEvent, DomUtil, FeatureGroup, Handler } from "leaflet";
...
const FitBoundsLeafletControl = Control.extend({
  fitFeatures: function (map, event) {
    ...
    if (this.options.textInputActive(event)) {
  ...
  onAdd: function (map) {
    ...
    map.addHandler(
      ...
      keyboardHandler(this.options.keyboardShortcutGroups.taskEditing.fitBounds.key, (event) =>
        this.fitFeatures(map, event),
      ),
    );
...
export const FitBoundsControl = createControlComponent(
  (props) => new FitBoundsLeafletControl(props),
);
