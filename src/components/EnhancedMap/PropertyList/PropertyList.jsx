...
const PropertyList = (props) => {
  ...
  const rows = _compact(
    _map(props.featureProperties, (value, key) => {
      if (_isObject(value)) {
        return null;
      }

      const link = !_isEmpty(tagInfo) ? (
        <a target="_blank" rel="noopener noreferrer" href={`${tagInfo}/keys/${key}`} title={key}>
        ...
        <tr key={key} className="property">
          <td
            className={classNames("name", {
              "mr-border-none mr-break-words mr-links-green-lighter mr-pr-3": darkMode,
            })}
          >
            {link}
          </td>
  ...
  return (
    <div className="feature-properties mr-ml-4" style={{ maxHeight: "300px", overflow: "auto" }}>
      ...
      <table
        className={classNames("property-list", {
          "mr-bg-transparent mr-text-white": darkMode,
          table: !darkMode,
        })}
