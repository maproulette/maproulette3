...
export const TaskMapContent = (props) => {
  ...
  const taskFeatures = () => {
    if (_get(props, "taskBundle.tasks.length", 0) > 0) {
      return featureCollection(
        _flatten(
          _compact(_map(props.taskBundle.tasks, (task) => _get(task, "geometries.features"))),
        ),
      ).features;
    }
  ...
  const handleKeyboardShortcuts = (event) => {
    ...
    if (props.textInputActive(event)) {
  ...
  const generateDirectionalityMarkers = () => {
    ...
    _each(allFeatures, (feature, featureIndex) => {
      ...
      const styles = AsSimpleStyleableFeature(
        feature,
        _get(props, "challenge.taskStyles"),
      ).getFinalLayerStyles();
  ...
  const overlayLayers = () => {
    let layers = buildLayerSources(
      ...
      _get(props, "user.settings.customBasemaps"),
