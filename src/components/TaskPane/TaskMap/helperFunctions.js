...
export const isClickOnMarker = (clickBounds, marker, map) => {
  ...
  const nw = map.layerPointToLatLng(
    new Point(markerPoint.x - iconOptions.iconAnchor[0], markerPoint.y - iconOptions.iconAnchor[1]),
  );
...
export const orderedFeatureLayers = (layers) => {
  ...
  const polygonLayers = _filter(
    layers,
    (l) => l.geometry.type === "Polygon" || l.geometry.type === "MultiPolygon",
  );
  const orderedPolygons =
    polygonLayers.length < 2
      ? polygonLayers
      : _sortBy(polygonLayers, (l) =>
          _reduce(
            polygonLayers,
            (count, other) => {
