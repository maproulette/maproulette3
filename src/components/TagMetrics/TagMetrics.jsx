...
export default class TagMetrics extends Component {
  buildTagStats = (metrics, totalTasks) => {
    const byTag = _map(_reverse(_sortBy(metrics, ["total", "tagName"])), (tagMetrics) => {
      return (
        <div
          className="mr-grid mr-grid-columns-1 mr-grid-gap-4"
          key={`${tagMetrics.tagType}:${tagMetrics.tagName}`}
        >
          {tagMetrics.tagName !== "" &&
            buildMetric(tagMetrics, totalTasks, tagMetrics.tagName, this.props)}
        </div>
      );
    });
    ...
    return (
      <Fragment>
        ...
        <div className={classNames("tag-metrics mr-mb-4")}>
          {reviewTags && totalTasks > 0 && this.buildTagStats(reviewTags, totalTasks)}
          {(!reviewTags || totalTasks === 0) && <FormattedMessage {...messages.noTags} />}
        </div>
        ...
        <div className={classNames("tag-metrics mr-mb-2")}>
          {taskTags && totalTasks > 0 && this.buildTagStats(taskTags, totalTasks)}
          {(!tagMetrics || totalTasks === 0) && <FormattedMessage {...messages.noTags} />}
        </div>
      </Fragment>
    );
  }
...
function buildMetric(metrics, total, description, props) {
  ...
  return (
    <div className="mr-grid mr-grid-columns-5 mr-grid-gap-2" title={tooltip}>
