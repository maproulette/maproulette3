  ...
  handleSubmit = async () => {
    ...
    if (this.state.characterCount < 100) {
      ...
      const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/issues`, {
        ...
        headers: {
          'Authorization': `token ${import.meta.env.VITE_GITHUB_ISSUES_API_TOKEN}`,
      ...
      if (response.ok) {
        ...
        const comment = `This challenge, challenge [#${challenge.id} - ${challenge.name}](${import.meta.env.VITE_URL}/browse/challenges/${challenge.id}) in project [#${challenge.parent.id} - ${challenge.parent.displayName}](${import.meta.env.VITE_URL}/browse/projects/${challenge.parent.id}), has been reported by [${this.props.user.osmProfile.displayName}](${import.meta.env.VITE_OSM_SERVER}/user/${encodeURIComponent(this.props.user.osmProfile.displayName)}). Please use [this GitHub issue](${issue_link}) to discuss. \n\n Report Content: \n ${this.state.value}`
