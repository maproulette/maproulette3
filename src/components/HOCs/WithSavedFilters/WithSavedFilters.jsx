...
import { messagesByPriority, TaskPriority } from "../../../services/Task/TaskPriority/TaskPriority";
import { messagesByStatus, TaskStatus } from "../../../services/Task/TaskStatus/TaskStatus";
import {
...
const WithSavedFilters = function (WrappedComponent, appSettingName) {
  return class extends Component {
    ...
    saveCurrentSearchFilters = (name, currentFilters) => {
      ...
      const settings = this.props.getUserAppSetting(this.props.user, appSettingName) || {};
      settings[this.props.pageId] = settings[this.props.pageId] || {};
      settings[this.props.pageId][name] = searchURL;
      this.props.updateUserAppSetting(this.props.user.id, { [appSettingName]: settings });
    };
    ...
    getBriefFilters = (savedFilters) => {
      ...
      return _compact(
        _map(criteria.filters, (value, key) => {
          let op = "=";
          let textValue = value;
    ...
    render() {
      return (
        <WrappedComponent
          ...
          cancelSavingFilters={() => this.setState({ savingFilters: false })}
          cancelManagingFilters={() => this.setState({ managingFilters: false })}
