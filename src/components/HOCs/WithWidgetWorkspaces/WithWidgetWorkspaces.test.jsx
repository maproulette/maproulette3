...
describe("WithWidgetWorkspaces", () => {
  ...
  it("can switch layouts, revert back to working layout if marked broken, and add new layout", async () => {
    const TestComponent = (props) => {
      return (
        <div>
          ...
          <button
            onClick={() => props.switchWorkspaceConfiguration(456, props.currentConfiguration)}
          >
            Switch Config
          </button>
          <button onClick={() => props.markWorkspaceConfigurationBroken()}>Mark Broken</button>
          <button onClick={() => props.addNewWorkspaceConfiguration(props.currentConfiguration)}>
            Add New Config
          </button>
          <button
            onClick={() => props.importWorkspaceConfiguration({}, props.currentConfiguration)}
          >
            Import Config
          </button>
        </div>
    ...
    const TestWrapped = WithWidgetWorkspaces(
      TestComponent,
      null,
      "userDashboard",
    ...
    vi.spyOn(services, "importWorkspaceConfiguration").mockReturnValue(
      Promise.resolve(importedConfig),
    );
