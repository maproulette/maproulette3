...
import { fetchReviewTagMetrics } from "../../../services/Task/TaskReview/TaskReview";
...
export const WithReviewTagMetrics = function (WrappedComponent) {
  return class extends Component {
    ...
    updateMetrics(props) {
      ...
      props
        .updateReviewTagMetrics(props.user?.id, props.reviewTasksType, props.reviewCriteria)
        .then(() => {
    ...
    render() {
      const totalTasks = this.props.reviewData?.totalCount ?? 0;
      return (
        <WrappedComponent
...
const mapStateToProps = (state) => {
  return { reviewTagMetrics: state.currentReviewTasks?.tagMetrics };
};
...
export default (WrappedComponent) =>
  connect(
    mapStateToProps,
    mapDispatchToProps,
  )(WithCurrentUser(WithReviewTagMetrics(WrappedComponent)));
