...
const WithCommandInterpreter = function (WrappedComponent, acceptedCommands = null) {
  return class extends Component {
    ...
    executeSearch = (commandString, searchType) => {
      ...
      const wasStandardSearch = executeCommand(
        this.props,
        commandString,
        searchType,
        false,
    ...
    deactivate = () => {
      executeCommand(
        ...
        (loading) => this.setState({ mapLoading: loading }),
        true,
        acceptedCommands,
      );
    };
    ...
    componentDidUpdate(prevProps) {
      if (
        prevProps?.searchQuery?.filters?.searchType !== this.props.searchQuery?.filters?.searchType
      ) {
        ...
        if (this.state.commandString) {
          ...
          this.setState({
            commandString: this.props.searchQuery?.filters?.project,
            searchType: this.props.searchQuery?.filters?.searchType,
          });
        } else {
          // Happens when task query is on url route
          this.setState({
            commandString: this.props.searchQuery?.filters?.task,
            searchType: this.props.searchQuery?.filters?.searchType,
          });
        }
      }
    ...
    render() {
      const query = this.state.commandString
        ? this.state.commandString
      ...
      return (
        <WrappedComponent
          {..._omit(this.props, ["searchQuery", "clearSearch", "executeSearch", "searchGroup"])}
...
const debouncedMapSearch = _debounce(
  (props, query, setLoading) => executeMapSearch(props, query, setLoading),
  1000,
  { leading: false },
);

const debouncedPlaceSearch = _debounce(
  (props, query, setLoading) => executePlaceSearch(props, query, setLoading),
  1000,
  { leading: false },
);
