...
import {
  ...
  REVIEW_STATUS_NOT_SET,
  META_REVIEW_STATUS_NOT_SET,
 ...
 */
export default function WithFilteredClusteredTasks(
  WrappedComponent,
  ...
  return class extends Component {
    ...
    toggleIncludedStatus = (status, exclusiveSelect = false) => {
      const includeStatuses = exclusiveSelect
        ...
        : Object.assign({}, this.state.includeStatuses, {
    ...
    toggleIncludedReviewStatus = (status, exclusiveSelect = false) => {
      const includeReviewStatuses = exclusiveSelect
        ...
        : Object.assign({}, this.state.includeReviewStatuses, {
    ...
    toggleIncludedMetaReviewStatus = (status, exclusiveSelect = false) => {
      const includeMetaReviewStatuses = exclusiveSelect
        ...
        : Object.assign({}, this.state.includeMetaReviewStatuses, {
    ...
    toggleIncludedPriority = (priority, exclusiveSelect) => {
      const includePriorities = exclusiveSelect
        ...
        : Object.assign({}, this.state.includePriorities, {
    ...
    taskPassesFilters = (
      ...
      return (
        ...
        (includeLocked ||
          !_isFinite(task.lockedBy) ||
          task.lockedBy === _get(this.props, "user.id"))
      );
    };
    ...
    setupFilters = () => {
      const savedFilters =
        useSavedFilters && savedFilterSettingName
      ...
      if (useURLFilters || loadFromSavedFilters) {
        const filteredTasks = this.filterTasks(
          criteria.filters.status || this.state.includeStatuses,
        ...
        this.setState(
          Object.assign(
            ...
            {
              includeStatuses,
              includeReviewStatuses: _fromPairs(
                _map(criteria.filters.reviewStatus, (status) => [status, true]),
              ),
              includeMetaReviewStatuses: _fromPairs(
                _map(criteria.filters.metaReviewStatus, (status) => [status, true]),
              ),
              includePriorities: _fromPairs(
                _map(criteria.filters.priorities, (priority) => [priority, true]),
              ),
            },
          ),
        );
    ...
    componentDidUpdate(prevProps) {
      if (
        !_isEqual(_get(prevProps[tasksProp], "tasks"), _get(this.props[tasksProp], "tasks")) ||
