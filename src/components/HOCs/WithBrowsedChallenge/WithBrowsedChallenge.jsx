...
export const WithBrowsedChallenge = function (WrappedComponent) {
  class _WithBrowsedChallenge extends Component {
     ...
     * @private
     */
    standardChallengeId = (props) => parseInt(_get(props, "match.params.challengeId"), 10);
     ...
     * @private
     */
    virtualChallengeId = (props) => parseInt(_get(props, "match.params.virtualChallengeId"), 10);
    ...
    updateBrowsedChallenge = (props) => {
      ...
      if (_isFinite(challengeId)) {
        if (
          _get(this.state, "browsedChallenge.id") !== challengeId ||
    ...
    render() {
      ...
      if (
        challengeId === _get(this.props, "clusteredTasks.challengeId") &&
      ...
      (challengeId) => {
        return dispatch(fetchChallenge(challengeId)).then((normalizedResults) => {
          if (
            ...
            _get(normalizedResults, `entities.challenges.${normalizedResults.result}.deleted`)
          ) {
            ...
            const projectId = _get(
              normalizedResults,
              `entities.challenges.${normalizedResults.result}.parent`,
            );
