...
export class ChallengeProgress extends Component {
  ...
  shouldComponentUpdate(nextProps) {
    ...
    if (
      !_isEqual(_get(nextProps, "challenge.actions", {}), _get(this.props, "challenge.actions", {}))
    ) {
      return true;
    }
          ...
          <ul key={index1} className={this.props.listClassName}>
            {_map(stats, (stat, index2) => (
              <li className="mr-flex mr-items-center" key={index1 + "-" + index2}>
                <span className="mr-text-lg mr-min-w-8 mr-text-right mr-text-pink">
                  {this.props.prominentCounts ? (
                    <Fragment>{stat[1].count >= 0 ? stat[1].count : "--"}</Fragment>
                  ) : (
                    <Fragment>
                  ...
                  <span className={this.props.prominentCounts ? "mr-text-base" : "mr-text-xs"}>
                    {this.props.prominentCounts ? (
                      ...
                      <Fragment>
                        ...
                        {taskActions.total >= 0 ? taskActions.total : "--"})
                      </Fragment>
                    )}
                  </span>
                </span>
  ...
  render() {
    ...
    if (_get(taskActions, "tasksWithTime", 0) > 0 && _get(taskActions, "totalTimeSpent", 0) > 0) {
    ...
    const orderedKeys = _map(
      orderedStatuses,
      (status) => localizedStatuses[keysByStatus[status]],
    ...
    const prioritizedCompletionProgress = _map(TaskPriority, (priority) => {
      if (taskPriorityActions && taskPriorityActions[priority]) {
        ...
        const completionPriorityData = this.calculateCompletionData(
          pActions,
          availableLabel,
          localizedStatuses,
        );
        const challengePriorityStats = this.calculateChallengeStats(
          pActions,
          orderedStatuses,
          localizedStatuses,
        );
