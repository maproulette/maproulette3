...
import { messagesByStatus, keysByStatus } from "../../services/Task/TaskStatus/TaskStatus";
import {
...
import { ViewCommentsButton, StatusLabel, makeInvertable } from "./TaskTableHelpers";
...
export class TaskAnalysisTableInternal extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    // the tag data.
    if (
      !_get(prevProps.addedColumns, "tags") &&
...
const setupColumnTypes = (props, taskBaseRoute, manager, data, openComments) => {
  ...
  columns.selected = {
    ...
    Cell: ({ value, original }) => {
      ...
      return props.highlightPrimaryTask && original.id === props.task?.id && !original.bundleId ? (
  ...
  columns.editBundle = {
    ...
    Cell: ({ row }) => {
      ...
      const alreadyBundled =
        row._original.bundleId && props.taskBundle?.bundleId !== row._original.bundleId;
  ...
  columns.tags = {
    ...
    Filter: ({ filter, onChange }) => {
      const preferredTags = _filter(
        _split(_get(props, "challenge.preferredTags"), ",").concat(
