  ...
  return class extends Component {
    render() {
      ...
      const toggleFilter = (filterName) => {
        ...
        searchquery +=
          tab !== 'users' ? `&hideUndiscoverable=${entityFilters.visible}` : ''
        searchquery +=
          tab !== 'users' ? `&hideArchived=${entityFilters.archived}` : ''
        searchquery +=
          tab === 'projects' ? `&virtual=${entityFilters.virtual}` : ''
      ...
      if (tab === 'challenges') {
        challenges = entityFilters.visible
          ? challenges.filter((c) => c.enabled)
          : challenges
        challenges = entityFilters.archived
          ? challenges.filter((c) => !c.isArchived)
          : challenges
        ...
        projects = entityFilters.visible
          ? projects.filter((p) => p.enabled)
          : projects
        projects = entityFilters.archived
          ? projects.filter((p) => !p.isArchived)
          : projects
        projects = entityFilters.virtual
          ? projects.filter((p) => p.isVirtual)
          : projects
