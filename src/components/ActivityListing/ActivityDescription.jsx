...
export const ActivityDescription = (props) => {
  const challengeName =
    props.entry.typeId === ActivityItemType.task ? props.entry.parentName : null;

  return (
    <div
      className={classNames("mr-flex mr-flex-col", {
    ...
    >
      ...
      <div
        className={
          props.simplified ? "mr-mt-2 mr-break-words" : "mr-mt-5 mr-text-base mr-break-words"
        }
      >
        <Link to={`/browse/challenges/${props.entry.parentId}`}>{challengeName}</Link>
      </div>
      {props.entry?.challenge?.general?.parent?.id && (
        <div className="mr-break-words mr-links-grey-light mr-mb-4">
          ...
          >
            {props.entry.challenge.general.parent.displayName ||
              props.entry.challenge.general.parent.name}
          </Link>
        </div>
      )}
      <div>
        ...
        <Link to={`/challenge/${props.entry.parentId}/task/${props.entry.itemId}`}>
